...
def main():
    d = runtime_entry.data; checks = list(d.CONCEPT_CHECKS_V112); by = {str(q.get("id") or ""): q for q in checks}
    failures, rows = [], []; expected = set(COHORT); rr = getattr(runtime_entry, "CONCEPT_CHECK_FINAL_CLINICAL_GATE_V179", {}); align = rr.get("task_alignment_v202") or {}
    ...
    for qid in sorted(expected):
        ...
        if not q.get("task_alignment_v202"): fail("missing_v202_marker")
        if len(words(prompt)) < 65 or "?" not in prompt: fail("weak_prompt:"+str(len(words(prompt))))
        if len(words(ans)) < 750: fail("weak_answer:"+str(len(words(ans))))
        ...
        if set(q.get("depth_layers_v202") or {}) != {"foundation","application","senior_decision"}: fail("missing_three_layer_depth")
        traps=q.get("common_traps_v202") or []
        if len(traps)<7 or any(len(words(x))<25 for x in traps): fail("weak_individualized_trap_reasoning")
        refs=q.get("source_refs_v202") or []; types=[x.get("type") for x in refs]
        if len(refs)<6 or types.count("textbook")<2 or "guideline" not in types or "systematic_review" not in types or "evidence" not in types or "review" not in types: fail("missing_traceable_source_mix")
        if not str(q.get("deliberate_review_v202") or "").strip(): fail("missing_deliberate_review_metadata")
        for label,terms in TERM_GROUPS.items():
            if not all(sem(term) in text for term in terms): fail("missing_semantic_group:"+label)
        if "CT is an anatomic tool, not a prerequisite" not in ans: fail("missing_airway_imaging_boundary")
        if "Awake flexible intubation is a strategy, not a guarantee" not in ans: fail("missing_awake_failure_boundary")
        if "selected clinically stable patients without a discrete drainable collection" not in ans: fail("missing_selective_conservative_boundary")
        if "do not swing to the opposite error" not in ans: fail("missing_antibiotics_only_boundary")
        if "Steroids are sometimes used as an adjunct" not in ans or "not an airway plan" not in ans: fail("missing_steroid_boundary")
    ...
    with open("V202_TASK_ALIGNMENT_AUDIT.json","w",encoding="utf-8") as f: json.dump({"expected_ids":sorted(expected),"runtime_alignment":align,"failures":failures,"items":rows},f,indent=2,ensure_ascii=False)
    print(f"V202_EXPECTED|{len(expected)}"); print(f"V202_REPAIRED|{len(repaired)}"); print(f"V202_FAILURES|{len(failures)}")
    for r in rows: print("V202_DEPTH_ITEM|{id}|domain={domain}|prompt={prompt_words}|answer={answer_words}|traps={trap_count}|sources={source_count}|topic={resolved_topic}".format(**r))
