"""Regression audit for the exact post-v38.1 production registry."""
...
from daily_curriculum_quality_v381 import CURVEBALL_OVERRIDES, DAILY_OVERRIDES
...
def main():
    ...
    print(f"V381_ITEMS|{len(items)}")
    print(f"V381_CONCEPTS|{len(concepts)}")
    print(f"V381_COMPLETE_SIX_STAGE|{sum(c == 6 for c in stages.values())}/{len(stages)}")
    print(f"V381_BLINDED_LABELED|{sum(bool(i.get('blind_case_label')) for i in blinded)}/{len(blinded)}")
    print(f"V381_DAILY_PAIRS_REPAIRED|{repaired_daily}")
    print(f"V381_CURVEBALLS_REPAIRED|{len(CURVEBALL_OVERRIDES)}")
    print(f"V381_CURVEBALLS_WITH_ANSWERS|{sum(bool(q.get('curveball_answer')) for q in curveballs)}/{len(curveballs)}")
    print(f"V381_FAILURES|{len(failures)}")
    for failure in failures[:100]:
        print("FAIL|" + failure)
    if failures:
        raise SystemExit(1)
    print("PASS: residual safety-boundary questions reveal decision-matched answers")
