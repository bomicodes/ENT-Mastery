"""v20.21 live-canonical Concept Check depth backlog gate."""
import json, os
from audit_concept_check_depth_backlog_v220 import main as _v220_main
from concept_check_depth_v221 import QIDS

def main():
    _v220_main()
    source, target = "V220_DEPTH_BACKLOG_AUDIT.json", "V221_DEPTH_BACKLOG_AUDIT.json"
    if not os.path.exists(source): raise SystemExit("v20.21 backlog gate did not receive v20.20 resolver output")
    with open(source, "r", encoding="utf-8") as f: report = json.load(f)
    markers = list(report.get("discovered_depth_markers") or [])
    failures = list(report.get("failures") or [])
    if "task_alignment_v221" not in markers: markers.append("task_alignment_v221")
    targets = set(QIDS)
    ...
    report["latest_depth_marker_version"] = max(int(report.get("latest_depth_marker_version") or 0), 221)
    report["audit_version"] = "v20.21"
    report["failures"] = failures
    with open(target, "w", encoding="utf-8") as f: json.dump(report, f, indent=2, ensure_ascii=False)
    print(f"V221_CANONICAL|{report.get('canonical_count')}")
    print(f"V221_UNTOUCHED_UNDER_75_WORDS|{report.get('untouched_candidate_count')}")
    print(f"V221_RESIDUAL_UNDER_75_WORDS|{report.get('residual_candidate_count')}")
    print("V221_DISCOVERED_DEPTH_MARKERS|" + ",".join(markers))
    print(f"V221_FAILURES|{len(failures)}")
