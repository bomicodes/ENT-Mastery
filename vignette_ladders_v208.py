"""v20.8/v20.9 — deliberate learning-ladder curation, Rhinology pass 4.

Reviews five more canonical foundations. Strong v14.4 cases are reused as the
application layer; only senior/chief decision questions are added. v20.9 removes
an accidental all-choice-B answer pattern without altering clinical content.
"""
...
_q("v208_rhi_turb_snr","Inferior Turbinate Hypertrophy",
   ...
   ["Repeat aggressive turbinectomy because persistent obstruction proves residual turbinate tissue","Perform Draf III","Identify and treat the dynamic nasal-valve mechanism rather than removing additional functioning turbinate tissue","Prescribe indefinite topical oxymetazoline"],2,
   ...
   ["Further resection targets the wrong mechanism and sacrifices functional mucosa.","Frontal sinusotomy does not address inspiratory nasal-valve collapse.","Correct. Treat the demonstrated valve collapse rather than escalating turbinate resection.","Chronic topical vasoconstrictor use risks rhinitis medicamentosa and is not structural treatment."],
...
_q("v208_rhi_anat_snr","Nasal Anatomy for Endoscopy",
   ...
   ["Stop, re-establish orientation from fixed landmarks and multiplanar CT/navigation as available, then proceed only when orbit and skull base are confidently localized","Continue dissecting until the anatomy becomes obvious","Use the microdebrider blindly because revision scar is usually safe","Convert the uncertain partition into a larger opening with forceps"],0,
   ...
   ["Correct. Losing orientation is an indication to stop and rebuild the three-dimensional map.","Dissection without localization turns uncertainty into skull-base or orbital risk.","Powered instrumentation can rapidly worsen an injury when the tissue boundary is uncertain.","Making an unknown structure larger is not a localization strategy."],
...
_q("v208_rhi_objective_snr","Objective Assessment of Nasal Function",
   ...
   ["Reject surgery because one objective airflow test is not severely abnormal","Diagnose malingering because symptom and airflow scores differ","Perform turbinectomy because rhinomanometry cannot assess obstruction","Integrate symptoms, dynamic examination, anatomy, and adjunctive testing; no single physiologic threshold should overrule a concordant clinical mechanism"],3,
   ...
   ["Objective testing is an adjunct rather than a universal veto threshold.","Symptom-test discordance is expected in a multidimensional sensory/airflow system and does not establish malingering.","A turbinate operation is inappropriate without evidence that turbinate tissue is the limiting mechanism.","Correct. Discordance should prompt synthesis, not automatic dismissal of either the patient or the test."],
...
_q("v208_rhi_smell_snr","Olfactory Dysfunction",
   ...
   ["Explain that surgery has no anatomic target, continue structured olfactory rehabilitation and safety counseling, and investigate new focal neurologic or unilateral red flags if they emerge","Offer ESS because anosmia alone proves occult sinus obstruction","Give prolonged antibiotics","Recommend permanent avoidance of odor exposure"],0,
   ...
   ["Correct. Match treatment to the sensorineural mechanism and reserve imaging/workup escalation for atypical features.","Normal objective sinonasal anatomy provides no surgically correctable target.","Antibiotics do not rehabilitate postviral olfactory neurons.","Structured odor exposure is therapeutic; blanket avoidance does not promote recovery."],
...
_q("v208_rhi_pedscrs_snr","Pediatric Chronic Rhinosinusitis",
   ...
   ["Proceed directly to adult-style complete ESS because the CT is diffuse","Perform frontal sinus obliteration","Evaluate for an underlying mucociliary disorder such as primary ciliary dyskinesia and coordinate systemic airway care while tailoring any sinus surgery to a defined goal","Assume adenoidectomy failure proves immunoglobulin deficiency"],2,
   ...
   ["Diffuse CT disease does not explain the multisystem phenotype or replace etiologic evaluation.","Frontal obliteration is not a routine escalation for pediatric inflammatory CRS.","Correct. The systemic mucociliary phenotype changes counseling, medical care, cultures, and the goals of surgery.","Adenoidectomy failure is nonspecific and does not establish a particular immune disorder."],
