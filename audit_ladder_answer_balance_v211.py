...
PREFIXES=("v209_","v210_","v212_","v213_","v216_","v218_","v219_","v220_","v221_","v222_")
...
for prefix in PREFIXES:
    ...
    if len(rows)<5: failures.append(f"{prefix}: expected at least 5 reviewed ladder rows, found {len(rows)}"); continue
    counts=Counter()
    for q in rows:
        ...
        counts[answer]+=1
    if len(counts)<3: failures.append(f"{prefix}: only {len(counts)} answer positions used: {dict(counts)}")
    if counts and max(counts.values())>(len(rows)+1)//2: failures.append(f"{prefix}: excessive answer-position concentration: {dict(counts)}")
