"""v18.8 hard gate for the next live-canonical Concept Check depth cohort."""
import json,re
import runtime_entry
from concept_check_board_repair_v177 import _find_module
from concept_check_depth_v188 import COHORT
TASK_TERMS={
...
def main():
 d=runtime_entry.data; checks=list(d.CONCEPT_CHECKS_V112); by={str(q.get('id') or ''):q for q in checks}; failures=[]; rows=[]; expected=set(COHORT); rr=getattr(runtime_entry,'CONCEPT_CHECK_FINAL_CLINICAL_GATE_V179',{}); align=rr.get('task_alignment_v188') or {}
 ...
 for qid in sorted(expected):
  ...
  if not q.get('task_alignment_v188'): fail('missing_v188_marker')
  ...
  if set(q.get('depth_layers_v188') or {})!={'foundation','application','senior_decision'}: fail('missing_three_layer_depth')
  traps=q.get('common_traps_v188') or []
  if len(traps)<2 or any(len(words(x))<8 for x in traps): fail('weak_individualized_trap_reasoning')
  if not str(q.get('deliberate_review_v188') or '').strip(): fail('missing_deliberate_review_metadata')
 ...
 with open('V188_TASK_ALIGNMENT_AUDIT.json','w',encoding='utf-8') as f: json.dump({'expected_ids':sorted(expected),'runtime_alignment':align,'failures':failures,'items':rows},f,indent=2,ensure_ascii=False)
 print(f'V188_EXPECTED|{len(expected)}'); print(f'V188_REPAIRED|{len(repaired)}'); print(f'V188_FAILURES|{len(failures)}')
