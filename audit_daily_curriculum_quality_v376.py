"""Regression audit for the exact post-v37.6 production registry."""
...
from daily_curriculum_quality_v376 import CURVEBALL_OVERRIDES, DAILY_OVERRIDES
...
def main():
    ...
    print(f"V376_ITEMS|{len(items)}")
    print(f"V376_CONCEPTS|{len(concepts)}")
    print(
        "V376_COMPLETE_SIX_STAGE|"
        f"{sum(count == 6 for count in stage_counts.values())}/{len(stage_counts)}"
    )
    print(
        "V376_BLINDED_LABELED|"
        f"{sum(bool(i.get('blind_case_label')) for i in blinded)}/{len(blinded)}"
    )
    print(f"V376_DAILY_PAIRS_REPAIRED|{repaired_daily}")
    print(f"V376_CURVEBALLS_REPAIRED|{len(CURVEBALL_OVERRIDES)}")
    print(
        "V376_CURVEBALLS_WITH_ANSWERS|"
        f"{sum(bool(q.get('curveball_answer')) for q in curveballs)}/{len(curveballs)}"
    )
    print(f"V376_FAILURES|{len(failures)}")
    for failure in failures[:100]:
        print("FAIL|" + failure)
    if failures:
        raise SystemExit(1)
    print("PASS: broad questions reveal direct, substantive answers in production")
