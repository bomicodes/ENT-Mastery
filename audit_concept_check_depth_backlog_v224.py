...
def main():
    ...
    if not os.path.exists(source): raise SystemExit("v20.24 backlog gate did not receive v20.23 resolver output")
    with open(source, "r", encoding="utf-8") as f: report = json.load(f)
    markers = list(report.get("discovered_depth_markers") or [])
    failures = list(report.get("failures") or [])
    if "task_alignment_v224" not in markers: markers.append("task_alignment_v224")
    targets = set(QIDS)
    ...
    print(f"V224_CANONICAL|{report.get('canonical_count')}")
    ...
    print("PASS: v20.24 preserves the exact live canonical backlog contract after ENT Fluids / Electrolytes / Nutrition depth")
