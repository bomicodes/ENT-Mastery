"""v20.31 live-canonical Concept Check backlog successor gate."""
import json
import os
from audit_concept_check_depth_backlog_v230 import main as _v230_main
from concept_check_depth_v231 import QIDS


def main():
    _v230_main()
    source, target = "V230_DEPTH_BACKLOG_AUDIT.json", "V231_DEPTH_BACKLOG_AUDIT.json"
    if not os.path.exists(source):
        raise SystemExit("v20.31 backlog gate did not receive v20.30 resolver output")
    with open(source, "r", encoding="utf-8") as f:
        report = json.load(f)
    failures = list(report.get("failures") or [])
    markers = list(report.get("discovered_depth_markers") or [])
    if "task_alignment_v231" not in markers:
        markers.append("task_alignment_v231")
    targets = set(QIDS)
    ...
    report["latest_depth_marker_version"] = max(int(report.get("latest_depth_marker_version") or 0), 231)
    report["audit_version"] = "v20.31"
    report["successor_of"] = "v20.30"
    report["newly_deepened_exact_live"] = list(QIDS)
    report["selection_policy"] = "prioritize clinically high-yield resident/board/OR gaps over lexical shortness; this cohort selects a Pediatric ENT airway-hearing-speech integration gap while avoiding concurrent Rhinology work"
    report["failures"] = failures
    with open(target, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    print(f"V231_CANONICAL|{report.get('canonical_count')}")
    print(f"V231_FAILURES|{len(failures)}")
    for failure in failures:
        print("FAIL|" + failure)
    if failures:
        raise SystemExit(1)
    print("PASS: v20.31 preserves the 325-topic backlog contract and removes only the exact live cleft/craniofacial target")
