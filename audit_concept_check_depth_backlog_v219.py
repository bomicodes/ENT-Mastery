"""v20.19 live-canonical Concept Check depth backlog gate."""
import json, os
from audit_concept_check_depth_backlog_v218 import main as _v218_main
from concept_check_depth_v219 import QIDS

def main():
    _v218_main()
    source,target="V218_DEPTH_BACKLOG_AUDIT.json","V219_DEPTH_BACKLOG_AUDIT.json"
    if not os.path.exists(source): raise SystemExit("v20.19 backlog gate did not receive v20.18 resolver output")
    ...
    if "task_alignment_v219" not in markers: markers.append("task_alignment_v219")
    targets=set(QIDS)
    ...
    report["discovered_depth_markers"]=markers; report["audit_version"]="v20.19"; report["failures"]=failures
    with open(target,"w",encoding="utf-8") as f: json.dump(report,f,indent=2,ensure_ascii=False)
    print(f"V219_CANONICAL|{report.get('canonical_count')}"); print(f"V219_UNTOUCHED_UNDER_75_WORDS|{report.get('untouched_candidate_count')}"); print(f"V219_RESIDUAL_UNDER_75_WORDS|{report.get('residual_candidate_count')}"); print("V219_DISCOVERED_DEPTH_MARKERS|"+",".join(markers)); print(f"V219_FAILURES|{len(failures)}")
