...
question generation. The newest cohort remains an explicit call because the fail-closed
release manifest verifies that the latest depth patch is truly executable in production.
"""
import re
...
def apply_final_clinical_gate_v179(checks, deep_modules, v6_item_id):
    ...
    results["post_alignment_reframed_v209"] = _reassert_clinical_contract(
        checks, alignment_v209.get("repaired", []), unresolved, "post_alignment_clinical_frame_v209"
    )
    ...
    results["post_alignment_reframed_v210"] = _reassert_clinical_contract(
        checks, alignment_v210.get("repaired", []), unresolved, "post_alignment_clinical_frame_v210"
    )
    return results
