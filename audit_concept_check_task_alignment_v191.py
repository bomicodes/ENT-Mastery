"""v19.1 hard gate for pediatric reconstruction/hearing and stroboscopy depth."""
import json,re
import runtime_entry
from concept_check_board_repair_v177 import _find_module
from concept_check_depth_v191 import COHORT
TASK_TERMS={
"cc-v112-rec-pediatric-otolaryngology-cleft-lip-palate-ent-surgical-fundamentals":["levator veli palatini","tympanostomy","velopharyngeal","fistula"],
"cc-v112-rec-pediatric-otolaryngology-microtia-reconstruction":["temporal-bone ct","jahrsdoerfer","bone-conduction","temporoparietal fascia"],
"cc-v112-rec-pediatric-otolaryngology-congenital-cmv-hearing-loss":["21 days","urine pcr","valganciclovir","cochlear-implant"],
...
def main():
 ...
 for qid in sorted(expected):
  ...
  if set(q.get('depth_layers_v191') or {})!={'foundation','application','senior_decision'}: fail('missing_three_layer_depth')
 ...
 with open('V191_TASK_ALIGNMENT_AUDIT.json','w',encoding='utf-8') as f: json.dump({'expected_ids':sorted(expected),'runtime_alignment':align,'failures':failures,'items':rows},f,indent=2,ensure_ascii=False)
 print(f'V191_EXPECTED|{len(expected)}'); print(f'V191_REPAIRED|{len(repaired)}'); print(f'V191_FAILURES|{len(failures)}')
 for r in rows: print('V191_DEPTH_ITEM|{id}|prompt={prompt_words}|answer={answer_words}|traps={trap_count}|topic={resolved_topic}'.format(**r))
