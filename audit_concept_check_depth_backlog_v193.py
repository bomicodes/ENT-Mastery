"""v19.3 deterministic post-completion Concept Check depth backlog audit.

Depth markers are discovered dynamically from live Concept Check records so
future cohorts cannot silently re-enter the untouched queue when the audit is
not edited in lockstep.
"""
...
def main():
 ...
 for q in checks:
  ...
  resolved.append((q,module,cid)); markers=depth_markers(q); discovered_markers.update(markers)
 ...
 with open("V193_DEPTH_BACKLOG_AUDIT.json","w",encoding="utf-8") as f: json.dump(report,f,indent=2,ensure_ascii=False)
 print(f"V193_CANONICAL|{len(canonical)}"); print(f"V193_CONCEPT_CHECKS|{len(checks)}"); print(f"V193_REVIEWED_FREE_RESPONSE|{reviewed_fr}"); print(f"V193_DEEPENED_CONCEPTS|{len(deepened_concepts)}"); print("V193_DISCOVERED_DEPTH_MARKERS|"+",".join(report["discovered_depth_markers"])); print(f"V193_UNTOUCHED_UNDER_75_WORDS|{len(primary)}"); print(f"V193_RESIDUAL_UNDER_75_WORDS|{len(residual)}")
 for r in primary[:12]: print("V193_CANDIDATE|{rank}|priority={priority_score}|answer={answer_words}|prompt={prompt_words}|{domain}|{canonical_topic}|{id}|{concept_id}".format(**r))
 for x in failures[:200]: print("FAIL|"+x)
 print(f"V193_FAILURES|{len(failures)}")
