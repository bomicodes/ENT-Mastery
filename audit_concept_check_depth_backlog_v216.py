"""v20.16 live-canonical Concept Check depth backlog gate."""
import json, os
from audit_concept_check_depth_backlog_v215 import main as _v215_main
from concept_check_depth_v216 import QIDS

def main():
    _v215_main()
    source,target="V215_DEPTH_BACKLOG_AUDIT.json","V216_DEPTH_BACKLOG_AUDIT.json"
    if not os.path.exists(source): raise SystemExit("v20.16 backlog gate did not receive v20.15 resolver output")
    ...
    if "task_alignment_v216" not in markers: markers.append("task_alignment_v216")
    targets=set(QIDS)
    ...
    report["discovered_depth_markers"]=markers; report["audit_version"]="v20.16"; report["failures"]=failures
    with open(target,"w",encoding="utf-8") as f: json.dump(report,f,indent=2,ensure_ascii=False)
    print(f"V216_CANONICAL|{report.get('canonical_count')}"); print(f"V216_UNTOUCHED_UNDER_75_WORDS|{report.get('untouched_candidate_count')}"); print(f"V216_RESIDUAL_UNDER_75_WORDS|{report.get('residual_candidate_count')}"); print("V216_DISCOVERED_DEPTH_MARKERS|"+",".join(markers)); print(f"V216_FAILURES|{len(failures)}")
