"""Regression audit for the exact post-v37.8 production registry."""
...
from daily_curriculum_quality_v378 import CURVEBALL_OVERRIDES, DAILY_OVERRIDES
...
def main():
    ...
    print(f"V378_ITEMS|{len(items)}")
    print(f"V378_CONCEPTS|{len(concepts)}")
    print(f"V378_COMPLETE_SIX_STAGE|{sum(c == 6 for c in stages.values())}/{len(stages)}")
    print(f"V378_BLINDED_LABELED|{sum(bool(i.get('blind_case_label')) for i in blinded)}/{len(blinded)}")
    print(f"V378_DAILY_PAIRS_REPAIRED|{repaired_daily}")
    print(f"V378_CURVEBALLS_REPAIRED|{len(CURVEBALL_OVERRIDES)}")
    print(f"V378_CURVEBALLS_WITH_ANSWERS|{sum(bool(q.get('curveball_answer')) for q in curveballs)}/{len(curveballs)}")
    print(f"V378_FAILURES|{len(failures)}")
    for failure in failures[:100]:
        print("FAIL|" + failure)
    if failures:
        raise SystemExit(1)
    print("PASS: residual questions and curveballs reveal complete decision-matched answers")
