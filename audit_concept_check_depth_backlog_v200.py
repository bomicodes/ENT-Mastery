...
Reuse the mature v19.9 resolver/priority contract while requiring the exact
Anaplastic Thyroid Cancer repair to leave both short-answer queues. Dynamic
marker discovery prevents later depth cohorts from becoming invisible.
"""
import json
import os
...
def main():
    _v199_main()
    source = "V199_DEPTH_BACKLOG_AUDIT.json"
    target = "V200_DEPTH_BACKLOG_AUDIT.json"
    if not os.path.exists(source):
        raise SystemExit("v20.0 backlog gate did not receive v19.9 resolver output")
    with open(source, "r", encoding="utf-8") as f:
        report = json.load(f)
    markers = report.get("discovered_depth_markers") or []
    failures = list(report.get("failures") or [])
    if "task_alignment_v200" not in markers:
        failures.append("missing_dynamic_depth_marker:task_alignment_v200")
    ...
    print("V200_DISCOVERED_DEPTH_MARKERS|" + ",".join(markers))
