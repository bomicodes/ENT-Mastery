...
PALLIATIVE_HN_REBUILD_V314 = {
    "palliative goals of care decision making in head neck cancer": {
        "recognize": (
            "Goal-concordant decision making, not a checklist of salvage procedures, is the central skill here. Palliative care can and should occur concurrently with disease-directed treatment; it is not synonymous with hospice or imminent death. Trigger structured goals-of-care work when disease is incurable or recurrent, treatment burden is rising, symptoms are uncontrolled, function/communication are threatened, or the patient faces a preference-sensitive choice such as morbid salvage surgery versus nonoperative care. Start by identifying what the patient understands, what outcomes matter most, and which tradeoffs are unacceptable."
        ),
        "localize": (
