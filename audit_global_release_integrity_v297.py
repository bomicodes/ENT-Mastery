"""v29.7 fail-closed manifest audit for ENT Mastery's global release gates.

In addition to discovering the newest Concept Check alignment/backlog audits and
adversarial OR rescue audit, this version proves that the newest versioned
Concept Check depth cohort is actually wired into both its current hard gate
and the production final-clinical-gate chain. A newly added depth cohort cannot
therefore trigger release CI while remaining silently unapplied or unaudited.
"""
...
def main() -> None:
    ...
    print(
        ...
        "Concept Check depth cohort, current backlog, OR rescue, all nine domain gates, "
        "strict canonical coverage, and answer balance"
    )
