"""v23.1 — full-domain Head & Neck Oncology learning-ladder hard gate."""
...
def main():
    ...
    for module in modules:
        ...
        print(f"HN_LADDER|{topic}|cases={len(linked)}|foundation={stage_counts['foundation']},application={stage_counts['application']},senior_decision={stage_counts['senior_decision']}|reviewed={int(reviewed)}")
    ...
    print(f"HN_CANONICAL_TOPICS|{len(modules)}"); print(f"HN_COMPLETE_TOPICS|{complete}"); print(f"HN_LADDER_GAPS|{len(failures)}")
    if failures:
        for failure in failures: print("HN_LADDER_FAILURE|"+failure)
        raise SystemExit(1)
    print("PASS: all 43 canonical Head & Neck Oncology topics have deliberate foundation, application, and senior-decision coverage")
