...
def apply_rhinology_fungal_boundary_depth_v353(data_module, app_module=None):
    ...
    for module in modules:
        ...
        module["deliberate_review_v353"] = {
            "foundation": "noninvasive versus tissue-invasive fungal disease and host/pathology distinctions",
            "application": "unilateral fungal-ball workup versus urgent biopsy/imaging pathway for suspected invasion",
            "senior_decision": "clearance versus emergency source control/systemic antifungal therapy, extension mapping and re-debridement decisions",
        }
        patched.append(topic)
    expected, actual = set(PATCHES), set(patched)
    if actual != expected:
        raise RuntimeError(f"v35.3 canonical patch-target mismatch: missing={sorted(expected-actual) or 'none'} unexpected={sorted(actual-expected) or 'none'}")
