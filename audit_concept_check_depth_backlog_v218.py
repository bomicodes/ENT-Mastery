"""v20.18 live-canonical Concept Check depth backlog gate."""
import json, os
from audit_concept_check_depth_backlog_v217 import main as _v217_main
from concept_check_depth_v218 import QIDS

def main():
    _v217_main()
    source,target="V217_DEPTH_BACKLOG_AUDIT.json","V218_DEPTH_BACKLOG_AUDIT.json"
    if not os.path.exists(source): raise SystemExit("v20.18 backlog gate did not receive v20.17 resolver output")
    ...
    if "task_alignment_v218" not in markers: markers.append("task_alignment_v218")
    targets=set(QIDS)
    ...
    report["discovered_depth_markers"]=markers; report["audit_version"]="v20.18"; report["failures"]=failures
    with open(target,"w",encoding="utf-8") as f: json.dump(report,f,indent=2,ensure_ascii=False)
    print(f"V218_CANONICAL|{report.get('canonical_count')}"); print(f"V218_UNTOUCHED_UNDER_75_WORDS|{report.get('untouched_candidate_count')}"); print(f"V218_RESIDUAL_UNDER_75_WORDS|{report.get('residual_candidate_count')}"); print("V218_DISCOVERED_DEPTH_MARKERS|"+",".join(markers)); print(f"V218_FAILURES|{len(failures)}")
