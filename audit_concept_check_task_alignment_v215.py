"""v20.15 exact-live Four-Gland Parathyroid Exploration task/source/semantic gate."""
import runtime_entry
from concept_check_board_repair_v177 import _find_module
from concept_check_depth_v215 import COHORT, QID, CID, TOPIC

SEMANTIC_GROUPS = {
    "selection_boundary": ("focused", "bilateral exploration", "multigland"),
    "rlN_embryology_map": ("posterior", "rln plane", "anterior"),
    "ectopic_search": ("paraesophageal", "thyrothymic", "supernumerary"),
    "remnant_perfusion": ("perfusion", "vascular", "autotransplant"),
    "iopth_baseline": ("pre-incision", "pre-excision", "baseline"),
    "renal_iopth_nuance": ("renal", "clearance", "normal glands"),
    "unsafe_search_bailout": ("blind deep", "reoperative plan", "rln"),
    "postop_rescue": ("hematoma", "hypocalcemia", "voice"),
}
...
def main():
    ...
    align=final.get("task_alignment_v215") or {}
    ...
    if q is None: failures.append("missing_target:"+QID)
        ...
        if not q.get("task_alignment_v215"): failures.append("missing_marker:task_alignment_v215")
        ...
        for field in ("depth_layers_v215","common_traps_v215","deliberate_review_v215","source_refs_v215"):
            if not q.get(field): failures.append("missing_metadata:"+field)
        traps=list(q.get("common_traps_v215") or [])
        if len(traps)<12 or len({str(x).strip().lower() for x in traps})<12: failures.append("trap_depth")
        refs=list(q.get("source_refs_v215") or []); reftext=" ".join(str(x.get("citation") or "") for x in refs if isinstance(x,dict)).lower()
        for anchor in ("cummings","pasha","k.j. lee","american association of endocrine surgeons","fifth international workshop","2026"):
    ...
    print("V215_TARGET|"+QID); print("V215_FAILURES|"+str(len(failures)))
    for failure in failures: print("FAIL|"+failure)
    if failures: raise SystemExit(1)
    print("PASS: v20.15 Four-Gland Parathyroid Exploration has exact-live linkage, textbook/current-source traceability, multigland anatomy, ioPTH baseline/renal nuance, perfusion protection, and bailout depth")
