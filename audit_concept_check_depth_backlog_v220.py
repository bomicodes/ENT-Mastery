...
def main():
    ...
    report["audit_version"] = "v20.20"
    report["failures"] = failures
    with open(target, "w", encoding="utf-8") as f: json.dump(report, f, indent=2, ensure_ascii=False)
    print(f"V220_CANONICAL|{report.get('canonical_count')}")
    print(f"V220_UNTOUCHED_UNDER_75_WORDS|{report.get('untouched_candidate_count')}")
    print(f"V220_RESIDUAL_UNDER_75_WORDS|{report.get('residual_candidate_count')}")
    print("V220_DISCOVERED_DEPTH_MARKERS|" + ",".join(markers))
    print(f"V220_FAILURES|{len(failures)}")
