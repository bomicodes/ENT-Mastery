"""v25.2 — strict 15-topic Laryngology deliberate-learning-ladder progress gate.

Protects exact canonical progress across the first three reviewed Laryngology batches.
...
def main():
    ...
    failures=[]
    if len(canonical)!=36:
        failures.append(f"expected 36 canonical Laryngology topics, found {len(canonical)}")
    if len(set(canonical))!=len(canonical):
        failures.append("duplicate canonical Laryngology topic names")
    ...
    for topic in PROTECTED_TOPICS:
        ...
        for q in rows:
            for err in _quality_errors(q):
                failures.append(f"{q.get('id')}: {err}")
    ...
    if failures:
        ...
        print("\n".join(failures))
        raise SystemExit(1)
    print("PASS: 15 exact canonical Laryngology topics retain complete reviewed ladders and quality contracts")
