"""v18.9 hard gate for the next live-canonical Concept Check depth cohort."""
import json,re
...
"cc-v112-rec-general-ent-emergencies-deep-neck-space-infection":["controlled airway","transcervical","carotid","mediastinitis"],
}
def words(v): return re.findall(r"\b\w+[\w'-]*\b",str(v or ""))
def main():
 d=runtime_entry.data; checks=list(d.CONCEPT_CHECKS_V112); by={str(q.get('id') or ''):q for q in checks}; failures=[]; rows=[]; expected=set(COHORT); rr=getattr(runtime_entry,'CONCEPT_CHECK_FINAL_CLINICAL_GATE_V179',{}); align=rr.get('task_alignment_v189') or {}
 ...
 for qid in sorted(expected):
  ...
  if not q.get('task_alignment_v189'): fail('missing_v189_marker')
  ...
  if set(q.get('depth_layers_v189') or {})!={'foundation','application','senior_decision'}: fail('missing_three_layer_depth')
  traps=q.get('common_traps_v189') or []
  if len(traps)<2 or any(len(words(x))<8 for x in traps): fail('weak_individualized_trap_reasoning')
  if not str(q.get('deliberate_review_v189') or '').strip(): fail('missing_deliberate_review_metadata')
 ...
 with open('V189_TASK_ALIGNMENT_AUDIT.json','w',encoding='utf-8') as f: json.dump({'expected_ids':sorted(expected),'runtime_alignment':align,'failures':failures,'items':rows},f,indent=2,ensure_ascii=False)
 print(f'V189_EXPECTED|{len(expected)}'); print(f'V189_REPAIRED|{len(repaired)}'); print(f'V189_FAILURES|{len(failures)}')
