...
def main():
    ...
    if not os.path.exists(source): raise SystemExit("v20.23 backlog gate did not receive v20.22 resolver output")
    with open(source, "r", encoding="utf-8") as f: report = json.load(f)
    markers = list(report.get("discovered_depth_markers") or [])
    failures = list(report.get("failures") or [])
    if "task_alignment_v223" not in markers: markers.append("task_alignment_v223")
    targets = set(QIDS)
    ...
    report["latest_depth_marker_version"] = max(int(report.get("latest_depth_marker_version") or 0), 223)
