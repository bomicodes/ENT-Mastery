"""v19.0 deterministic post-completion Concept Check depth backlog audit."""
from collections import Counter
import json,re
import runtime_entry
from concept_check_board_repair_v177 import _find_module
DATA=runtime_entry.data
MARKERS=("task_alignment_v180","task_alignment_v181","task_alignment_v182","task_alignment_v183","task_alignment_v184","task_alignment_v185","task_alignment_v186","task_alignment_v187","task_alignment_v188","task_alignment_v189","task_alignment_v190")
...
def main():
 ...
 report={"canonical_count":len(canonical),"concept_check_count":len(checks),"reviewed_free_response_count":reviewed_fr,"deepened_v180_v190_concept_count":len(deepened_concepts),"individually_deepened_questions_under_75_words":individually_deepened,"untouched_candidate_count":len(primary),"residual_candidate_count":len(residual),"selection_contract":"exact live canonical IDs; primary untouched concepts then residual weak siblings; priority desc, answer words asc, prompt words asc, concept_id, question id","failures":failures,"candidates":primary,"residual_candidates":residual}
 with open("V190_DEPTH_BACKLOG_AUDIT.json","w",encoding="utf-8") as f: json.dump(report,f,indent=2,ensure_ascii=False)
 print(f"V190_CANONICAL|{len(canonical)}"); print(f"V190_CONCEPT_CHECKS|{len(checks)}"); print(f"V190_REVIEWED_FREE_RESPONSE|{reviewed_fr}"); print(f"V190_DEEPENED_CONCEPTS|{len(deepened_concepts)}"); print(f"V190_UNTOUCHED_UNDER_75_WORDS|{len(primary)}"); print(f"V190_RESIDUAL_UNDER_75_WORDS|{len(residual)}")
 for r in primary[:12]: print("V190_CANDIDATE|{rank}|priority={priority_score}|answer={answer_words}|prompt={prompt_words}|{domain}|{canonical_topic}|{id}|{concept_id}".format(**r))
 for x in failures[:200]: print("FAIL|"+x)
 print(f"V190_FAILURES|{len(failures)}")
