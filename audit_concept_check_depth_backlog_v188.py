"""v18.8 deterministic post-completion Concept Check depth backlog audit."""
...
def main():
 ...
 report={"canonical_count":len(canonical),"concept_check_count":len(checks),"reviewed_free_response_count":reviewed_fr,"deepened_v180_v188_concept_count":len(deepened_concepts),"individually_deepened_questions_under_75_words":individually_deepened,"untouched_candidate_count":len(primary),"residual_candidate_count":len(residual),"selection_contract":"exact live canonical IDs; primary untouched concepts then residual weak siblings; priority desc, answer words asc, prompt words asc, concept_id, question id","failures":failures,"candidates":primary,"residual_candidates":residual}
 with open("V188_DEPTH_BACKLOG_AUDIT.json","w",encoding="utf-8") as f: json.dump(report,f,indent=2,ensure_ascii=False)
 print(f"V188_CANONICAL|{len(canonical)}"); print(f"V188_CONCEPT_CHECKS|{len(checks)}"); print(f"V188_REVIEWED_FREE_RESPONSE|{reviewed_fr}"); print(f"V188_DEEPENED_CONCEPTS|{len(deepened_concepts)}"); print(f"V188_UNTOUCHED_UNDER_75_WORDS|{len(primary)}"); print(f"V188_RESIDUAL_UNDER_75_WORDS|{len(residual)}")
 for r in primary[:12]: print("V188_CANDIDATE|{rank}|priority={priority_score}|answer={answer_words}|prompt={prompt_words}|{domain}|{canonical_topic}|{id}|{concept_id}".format(**r))
 for x in failures[:200]: print("FAIL|"+x)
 print(f"V188_FAILURES|{len(failures)}")
