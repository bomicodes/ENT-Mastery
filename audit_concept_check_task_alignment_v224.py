"""v20.24 exact-live ENT Fluids / Electrolytes / Nutrition task/source/semantic gate."""
...
def main():
    ...
    checks=list(data.CONCEPT_CHECKS_V112); deep_modules=data.DEEP_MODULES_V6; v6_item_id=data._v6_item_id
    final=getattr(runtime_entry,"CONCEPT_CHECK_FINAL_CLINICAL_GATE_V179",{}) or {}
    by={str(q.get("id") or ""):q for q in checks or []}; failures=[]
    ...
    for qid in QIDS:
        ...
        if topic != expected.get("canonical_topic") or cid != expected.get("concept_id") or q.get("concept_id") != expected.get("concept_id"): failures.append("canonical_link:"+qid)
        ...
        refs=list(q.get("source_refs_v224") or []); reftext=" ".join(str(x.get("citation") or "") for x in refs if isinstance(x,dict)).lower()
    ...
    print("PASS: v20.24 ENT fluids/electrolytes/nutrition has exact-live linkage, resuscitation/maintenance separation, sodium/electrolyte rescue, refeeding safeguards, enteral-first nutrition and explicit stop/reassess reasoning")
