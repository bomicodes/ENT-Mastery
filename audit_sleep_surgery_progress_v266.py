...
DOMAIN="Sleep Surgery"; STAGES={"foundation","application","senior_decision"}
...
def main():
 ...
 for topic in PROTECTED_TOPICS:
  ...
  rows=[q for q in by_cid.get(cid,[]) if q.get("ladder_reviewed")]; stages={q.get("learning_stage") for q in rows if q.get("learning_stage") in STAGES}
