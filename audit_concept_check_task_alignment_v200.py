...
def main():
    ...
    for qid in sorted(expected):
        ...
        if len(refs) < 5 or not any(x.get("type") == "textbook" for x in refs) or not any(x.get("type") == "guideline" for x in refs) or not any(x.get("type") == "regulatory" for x in refs): fail("missing_traceable_source_mix")
    ...
    with open("V200_TASK_ALIGNMENT_AUDIT.json", "w", encoding="utf-8") as f: json.dump({"expected_ids": sorted(expected), "runtime_alignment": align, "failures": failures, "items": rows}, f, indent=2, ensure_ascii=False)
    print(f"V200_EXPECTED|{len(expected)}"); print(f"V200_REPAIRED|{len(repaired)}"); print(f"V200_FAILURES|{len(failures)}")
